...
import com.jesen.andsockettolk.clink.net.jesen.clink.utils.ByteUtils;
import com.jesen.andsockettolk.constants.UDPConstants;
        ...
        public void run() {
            ...
            try {
                ...
                while (!done) {
                    ...
                    // 收到客户端的消息检测长度，header + 2字节的cmd + 客户端的端口int类型占4
                    ...
                    // 判断合法性
                    // 广播的cmd我们自定义为1，下面Server回送Client的cmd定义为2
                    if (cmd == 1 && responsePort > 0) {
                        // 构建一份回送数据
                        ByteBuffer byteBuffer = ByteBuffer.wrap(buffer);
                        byteBuffer.put(UDPConstants.HEADER);
                        byteBuffer.putShort((short) 2); // cmd定义为2
                        byteBuffer.putInt(port); // 把server端口号回送给Client，让Client用于发送TCP
                        byteBuffer.put(sn);
